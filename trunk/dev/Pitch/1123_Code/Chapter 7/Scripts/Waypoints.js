...
public function Awake() : void {
	...
	int_wayIndex = 0;
	int_nextIndex = 1;
	//Checking if not moving by order get the random index of waypoint
...
//To setup the start position of enemy at the same 
public function StartPosition() : Vector3 {
	return waypoints[0].position;
}

//Return the direction of the enemy toward the waynext
...
public function AwayFromWaypoint (_AI : Transform, _distance : float) : boolean {
	var f_distance : float = Vector3.Distance(_AI.position, waypoints[int_nextIndex].position);
	if (f_distance >= _distance) {
		return true;
	} else {
		return false;
	}
	...
	if (waypointGizmos != null) {
		if (orderDirection == true) {
			...
			for (var i : int = 0; i < waypointGizmos.Length; i++) {
				...
				if (i == (waypointGizmos.Length-1)) {
					Gizmos.DrawLine(waypointGizmos[i].position, waypointGizmos[0].position);
				} else {
					Gizmos.DrawLine(waypointGizmos[i].position, waypointGizmos[i+1].position);
				}
